...
export const juzRanges: JuzRange[] = [
...
export const calculateJuzByVerseRange = (surahId: number, verseId: number): number => {
    for (const range of juzRanges) {
        const startSurah = range.start.surah;
        const startVerse = range.start.verse;
        const endSurah = range.end.surah;
        const endVerse = range.end.verse;
        
        if (surahId > startSurah || (surahId === startSurah && verseId >= startVerse)) {
            if (surahId < endSurah || (surahId === endSurah && verseId <= endVerse)) {
                return range.juz;
            }
        }
    }
